...
router.get(
  '/', 
  validateRequest(goalListSchema), 
  GoalController.getAllGoals
);

router.get(
  '/dashboard', 
  GoalController.getGoalDashboard
);

router.get(
  '/:id', 
  validateRequest(goalIdParamSchema), 
  GoalController.getGoalById
);

router.patch(
  '/add-amount/:id', 
  validateRequest(addRemoveAmountSchema), 
