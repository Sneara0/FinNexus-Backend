...
const getGoalById = catchAsync(async (req: Request, res: Response) => {
  ...
  const goal = await GoalService.getGoalById(id as string, userId);
...
// 4. গোলে টাকা যোগ করা
// ========================================
const addToGoal = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  const { id } = req.params;
  const { amount } = req.body;
  
  const goal = await GoalService.addToGoal(id as string, userId, Number(amount));
...
// ========================================
const removeFromGoal = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  const { id } = req.params;
  const { amount } = req.body;
  
  const goal = await GoalService.removeFromGoal(id as string, userId, Number(amount));
...
// ========================================
const completeGoal = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  const { id } = req.params;
  
  const goal = await GoalService.completeGoal(id as string, userId);
...
const updateGoal = catchAsync(async (req: Request, res: Response) => {
  ...
  const goal = await GoalService.updateGoal(id as string, userId, {
    name,
    targetAmount,
...
// 9. গোল ড্যাশবোর্ড
// ========================================
const getGoalDashboard = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
