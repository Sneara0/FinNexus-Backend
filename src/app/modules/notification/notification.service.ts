import { NotificationType, Prisma, Notification as DbNotification } from '../../../generated/prisma/index.js';
...
export class NotificationService {
   ...
   * ১. নোটিফিকেশন তৈরি
   */
  static async createNotification(userId: string, data: INotificationPayload): Promise<DbNotification> {
   ...
   */
  static async getUserNotifications(userId: string, query: { page?: number; limit?: number; type?: NotificationType; isRead?: boolean }) {
   ...
   */
  private static async getVerifiedNotification(id: string, userId: string) {
   ...
   */
  static async createBudgetAlert(userId: string, category: string, spent: number, limit: number) {
  ...
  static async sendBudgetAlert(userId: string, details: { category: string; spent: number; budget: number }) {
     return this.createBudgetAlert(userId, details.category, details.spent, details.budget);
  }

  static async sendGoalMilestone(userId: string, goalName: string) {
