import { } from "@prisma/client"; // ✅ সরাসরি প্রিজমা ক্লায়েন্ট থেকে ইম্পোর্ট
import { prisma } from "../../lib/prisma.js"; // ✅ আপনার সেন্ট্রাল প্রিজমা ক্লায়েন্ট
import { CategoryRule, TransactionCategory, TransactionType } from "../../../generated/prisma/index.js";
...
export class CategoryRuleService {
  ...
  static async getRuleById(id: string, userId: string) {
    ...
    if (!rule) throw new Error('রুল পাওয়া যায়নি');
    return rule;
  }

  // ৪. রুল আপডেট
  static async updateRule(id: string, userId: string, data: Partial<CategoryRule>) {
  ...
  static async autoDetectCategory(description: string, userId: string) {
  ...
  // ৭. বাল্ক ডিলিট
  static async bulkDeleteRules(ruleIds: string[], userId: string) {
  ...
  static async toggleRuleStatus(id: string, userId: string) {
  ...
  // ৯. রুল পরিসংখ্যান
  static async getRuleStats(userId: string) {
  ...
  static async getSuggestedRules(userId: string) {
