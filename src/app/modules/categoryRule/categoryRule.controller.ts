...
export const getRuleById = catchAsync(async (req: Request, res: Response) => {
  ...
  const rule = await CategoryRuleService.getRuleById(id as string, userId);
...
export const updateRule = catchAsync(async (req: Request, res: Response) => {
  ...
  const rule = await CategoryRuleService.updateRule(id as string, userId, {
    keyword,
    category,
    type,
    priority,
...
export const deleteRule = catchAsync(async (req: Request, res: Response) => {
  ...
  await CategoryRuleService.deleteRule(id as string, userId);
...
// ========================================
export const autoDetectCategory = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
...
// 8. রুল স্ট্যাটাস টগল
// ========================================
export const toggleRuleStatus = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  const { id } = req.params;
  
  const rule = await CategoryRuleService.toggleRuleStatus(id as string, userId);
...
// ========================================
export const getRuleStats = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
...
// 10. সাজেস্টেড রুল
// ========================================
export const getSuggestedRules = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
