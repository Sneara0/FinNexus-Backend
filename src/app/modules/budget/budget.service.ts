...
export class BudgetService {
  ...
  static async getAllBudgets(
    ...
    if (period && period !== 'all') where.period = period as BudgetPeriod;
    if (category && category !== 'all') where.category = category as TransactionCategory;
  ...
  static async getBudgetById(budgetId: string, userId: string) {
    ...
    if (!budget) throw new Error('বাজেট পাওয়া যায়নি');
  ...
  // ৫. বাজেট আপডেট
  // ========================================
  static async updateBudget(budgetId: string, userId: string, data: any) {
    const budget = await prisma.budget.findFirst({
      where: { id: budgetId, userId }
    });
