import { Prisma, TransactionCategory, TransactionType } from '../../../generated/prisma/edge.js';
...
export class TransactionService {
  ...
  static async createTransaction(userId: string, data: any) {
    ...
    if (!category) category = TransactionCategory.OTHER;
  ...
  // ========================================
  static async getAllTransactions(userId: string, filters: any) {
    ...
    const where: Prisma.TransactionWhereInput = {
      ...
      ...(category && category !== 'all' && { category: category as TransactionCategory }),
  ...
  static async getTransactionById(id: string, userId: string) {
    ...
    if (!transaction) throw new Error('ট্রানজেকশন পাওয়া যায়নি');
    return transaction;
  }
  ...
  // 4. ট্রানজেকশন আপডেট
  // ========================================
  static async updateTransaction(transactionId: string, userId: string, data: any) {
    const existing = await prisma.transaction.findFirst({ where: { id: transactionId, userId } });
    if (!existing) throw new Error('ট্রানজেকশন পাওয়া যায়নি');
  ...
  // ========================================
  static async getTransactionSummary(userId: string, period: string) {
  ...
  // ========================================
  static async getCategoryAnalysis(userId: string, startDate: string, endDate: string) {
  ...
  // 8. বাল্ক ইম্পোর্ট
  // ========================================
  static async bulkImportTransactions(userId: string, transactions: any[]) {
  ...
  // 9. বাজেট এলার্ট চেক
  // ========================================
  static async checkBudgetAlert(userId: string, category: TransactionCategory) {
    const now = new Date();
