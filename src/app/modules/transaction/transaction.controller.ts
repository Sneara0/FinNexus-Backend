...
export const createTransaction = catchAsync(async (req: Request, res: Response) => {
  ...
  const {
    type, amount, description, date, currency,
    location, tags, receiptUrl, isRecurring, category
  } = req.body;
...
export const getAllTransactions = catchAsync(async (req: Request, res: Response) => {
  ...
  const filters = {
    ...
    startDate: req.query.startDate as string, // ✅ string হিসেবে রাখা হয়েছে যাতে সার্ভিসে এরর না দেয়
...
export const updateTransaction = catchAsync(async (req: Request, res: Response) => {
  ...
  const transaction = await TransactionService.updateTransaction(id as string, userId, {
    ...updateData,
...
// 6. ট্রানজেকশন সামারি
// ========================================
export const getTransactionSummary = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
...
// ========================================
export const getCategoryAnalysis = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
...
// ========================================
export const bulkImportTransactions = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
