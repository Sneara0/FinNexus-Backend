...
export class SubscriptionService {
  ...
  // 1. সাবস্ক্রিপশন তৈরি
  // ========================================
  static async createSubscription(userId: string, data: {
    name: string;
    amount: number;
    currency?: string;
    billingCycle: string;
  ...
  // ========================================
  static async reactivateSubscription(subscriptionId: string, userId: string) {
    const existingSubscription = await prisma.subscription.findFirst({
      where: { id: subscriptionId, userId }
    });
  ...
  // ========================================
  private static calculateNextBilling(startDate: Date, billingCycle: string): Date {
  ...
  static async getSubscriptionSummary(userId: string) {
    ...
    const monthlyTotal = subscriptions.reduce((sum, sub) => {
      ...
      else if (sub.billingCycle === 'weekly') amount = sub.amount * 4.34; // Average weeks in month
      return sum + amount;
    }, 0);
